...
const initializeInstance = async (req) => {
  try {
    const res = await fetch(
      `${baseUrl}/instance/init?key=${req.query.key}&token=${process.env.TOKEN}`
    );
    const JSONResponse = await res.json();
    return JSONResponse;
  } catch (err) {
...
app.post("/api/whatsapp/init", async (req, res) => {
  try {
    ...
    const response = await fetch(
      `${baseUrl}/instance/init?key=${key}&token=${process.env.TOKEN}`
    );
...
app.get("/api/whatsapp/qr", async (req, res) => {
  try {
    ...
    const response = await fetch(`${baseUrl}/instance/qrbase64?key=${key}`);
    const data = await response.json();
...
app.delete("/api/whatsapp/logout", async (req, res) => {
  try {
    ...
    const response = await fetch(`${baseUrl}/instance/logout?key=${key}`, {
      method: "DELETE",
    });
...
app.get("/api/whatsapp/status", async (req, res) => {
  try {
    ...
    const response = await fetch(`${baseUrl}/instance/info?key=${key}`);
    const data = await response.json();
    ...
    try {
      const qrRes = await fetch(`${baseUrl}/instance/qrbase64?key=${key}`);
      const qrData = await qrRes.json();

      qrAvailable = !!qrData?.qrcode;
    } catch {
      qrAvailable = false;
    }
      ...
      .then((response) => {
        ...
        if (response.error) {
          ...
          initializeInstance(req)
            .then((res) => {
              console.log(
                "instance reinitilaized, checking if instance is active"
              );
          ...
          setTimeout(() => {
            console.log(
              "Instance not active yet, check will commence again in 10 secs"
            );
          ...
          if (
            ...
            response.instance_data.user &&
            Object.keys(response.instance_data.user).length > 0
          ) {
            resolve(response);
          } else {
            setTimeout(() => {
...
const sendDeliveryReport = async (payload) => {
  try {
    const res = await fetch(
      `https://churchplusv3coreapi.azurewebsites.net/WhatsAppDeliveryReport`,
      {
        method: "POST",
        body: JSON.stringify(payload),
        headers: {
...
app.get("/scanQRCode", async (req, resp) => {
  try {
    const res = await fetch(
      `${baseUrl}/instance/qrbase64?key=${req.query.key}`
    );
    const JSONResponse = await res.json();
    resp.send(JSONResponse);
  } catch (err) {
...
app.get("/groups/getAllWhatsappGroups", async (req, resp) => {
  try {
    const res = await fetch(
      `${baseUrl}/group/getallgroups?key=${req.query.key}`
    );
    const JSONResponse = await res.json();
    resp.send(JSONResponse);
  } catch (err) {
...
const sendText = async (req) => {
  ...
  for (let i = 0; i < recipients.length; i += batchSize) {
    ...
    const batchPromises = batch.map((item) =>
      limit(async () => {
        ...
        try {
          ...
          console.error(
            `Error sending message to ${item.phoneNumber}:`,
            error.message
          );
    ...
    if (i + batchSize < recipients.length) {
      console.log(
        `Delaying for ${delayMs / 1000} seconds before the next batch...`
      );
      await delay(delayMs);
    }
  }
      ...
      const batchPromises = batch.map((item) =>
        limit(async () => {
          ...
          try {
            ...
            console.error(
              `Error sending image message to ${item.phoneNumber}:`,
              error.message
            );
            return { success: false, error: error.message, recipient: item };
          }
      ...
      const batchPromises = batch.map((item) =>
        limit(async () => {
          ...
          try {
            ...
            console.error(
              `Error sending video message to ${item.phoneNumber}:`,
              error.message
            );
            return { success: false, error: error.message, recipient: item };
          }
